...
def BuscarPaciente(rut):
    ...
    def altaMedica(rut):
        ...
        listaPacientesAM.adicionarFinal(Paciente(listaEnlazada.buscarPorIterar(rut).getNombre(),listaEnlazada.buscarPorIterar(rut).getRut(),listaEnlazada.buscarPorIterar(rut).getSexo(),listaEnlazada.buscarPorIterar(rut).getDireccion(),listaEnlazada.buscarPorIterar(rut).getDiagnosticoI(),listaEnlazada.buscarPorIterar(rut).getEstado(),listaEnlazada.buscarPorIterar(rut).getDespacho(),True ,listaEnlazada.buscarPorIterar(rut).getNombreM(),listaEnlazada.buscarPorIterar(rut).getRutM(),listaEnlazada.buscarPorIterar(rut).getDiagnostico(),listaEnlazada.buscarPorIterar(rut).getTratamiento(),listaEnlazada.buscarPorIterar(rut).getIdEspecialista(),listaEnlazada.buscarPorIterar(rut).getNombreEspecialidad()))
    ...
    if listaEnlazada.buscarPorIterar(rut) == "Paciente no registrado":
        ...
        cDireccion.set(listaEnlazada.buscarPorIterar(rut).getDireccion())
        ...
        imgamC= Image.open('images/btnAM.png')
        imgamC = ImageTk.PhotoImage(imgamC)
        btnAMC = Button(info, image=imgamC, borderwidth=0, bg="#EDF0F2", command =lambda: [altaMedica(rutS),close_window(info)]  )
        btnAMC.place(x = 260, y=300)
...
Label(root, text="Direccion:" ,bg = "#0026fe",fg ="white",font=("Aharoni",12,'bold')).place(x=230,y=260)
direccion = Entry(root, width=40)
direccion.place(x=360,y=260)
...
despacho = StringVar()
trauma = Radiobutton(root,  text="Traumatologia",variable=despacho,value="traumatologia",bg = "#0026fe",font=("Aharoni",10,'bold'))
trauma.place(x=360,y=350)
trauma.select()

neuro = Radiobutton(root,  text="Neurologia",variable=despacho,value="neurologia",bg = "#0026fe",fg ="black",font=("Aharoni",10,'bold'))
neuro.place(x=360,y=370)

cardio = Radiobutton(root,  text="Cardiologia", variable=despacho,value="cardiologia",bg = "#0026fe",fg ="black",font=("Aharoni",10,'bold'))
...
imgAgregar= ImageTk.PhotoImage(imgAgregar)
agregar = Button(root, image=imgAgregar, borderwidth=0, bg = "#0026fe", command=lambda: [distribuirListas(nombre.get(),rut.get(),sexo.get(),direccion.get(),diagI.get(),estado.get(),despacho.get()), ActualizaTabla()])
...
imgAgrega= ImageTk.PhotoImage(imgAgrega)
agregar = Button(root, image=imgAgrega,bg = "light grey", borderwidth=0, command=lambda: [BuscarPaciente(buscaPaciente.get()), buscaPaciente.delete(0, 'end')])
